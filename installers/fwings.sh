...
MYSQL_DBHOST_USER="${MYSQL_DBHOST_USER:-faliactyldbhost}"
...
firewall_ports() {
  output "Opening port 22 (SSH), 8080 (FWings Port), 2022 (FWings SFTP Port)"
...
perform_install() {
  output "Installing faliactyl wings.."
  dep_install
  systemctl enable fwings
